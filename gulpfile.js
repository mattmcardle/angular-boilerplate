...
gulp.task("gen-ts-refs", function() {
	...
	var sources = gulp.src([config.allTypeScript, config.ignoreTypeScript], { read: false });
...
/**
 * Inject generated into index.html
 */
...
gulp.task("compile-ts", ["gen-ts-refs"], function() {
	...
	// App Files
	var sourceTsFiles = [config.moduleFiles,
...
gulp.task("watch", function() {
    ...
    gulp.watch([config.allLess], ["inject-less"]);
});

gulp.task("build", ["ts-lint", "inject-ts", "inject-less"]);
gulp.task("default", ["ts-lint", "inject-ts", "inject-less", "watch", "serve"]);